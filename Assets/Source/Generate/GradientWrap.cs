...
public class GradientWrap
{
	...
	{
		L.BeginClass(typeof(Gradient), typeof(UnityEngine.UI.BaseMeshEffect));
	...
	static int ModifyMesh(IntPtr L)
	{
		try
		{
			...
			if (count == 2 && TypeChecker.CheckTypes(L, 1, typeof(Gradient), typeof(UnityEngine.Mesh)))
			{
				Gradient obj = (Gradient)ToLua.ToObject(L, 1);
				UnityEngine.Mesh arg0 = (UnityEngine.Mesh)ToLua.ToObject(L, 2);
				obj.ModifyMesh(arg0);
				return 0;
			}
			else if (count == 2 && TypeChecker.CheckTypes(L, 1, typeof(Gradient), typeof(UnityEngine.UI.VertexHelper)))
			{
				Gradient obj = (Gradient)ToLua.ToObject(L, 1);
			...
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to method: Gradient.ModifyMesh");
			}
		}
		catch(Exception e)
		{
